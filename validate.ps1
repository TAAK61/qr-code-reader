...
if ($LASTEXITCODE -eq 0 -and (Test-Path "build\libs\qr-code-reader-2.0-dev-all.jar")) {
    Write-Host "  ✓ JAR créé avec succès" -ForegroundColor Green
    $jarSize = (Get-Item "build\libs\qr-code-reader-2.0-dev-all.jar").Length / 1MB
...
if (Test-Path "build\libs\qr-code-reader-2.0-dev-all.jar") {
    $execResult = & java -jar "build\libs\qr-code-reader-2.0-dev-all.jar" 2>&1
    if ($execResult -match "QR Code Reader") {
...
if ($errors -eq 0) {
    ...
    Write-Host ""    Write-Host "Commandes utiles:" -ForegroundColor White
    Write-Host "  java -jar build\libs\qr-code-reader-2.0-dev-all.jar <image>" -ForegroundColor Cyan
} else {
