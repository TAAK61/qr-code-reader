...
/**
 * Plugin for reading EAN-13 barcodes
 * Task 71: Add support for additional barcode formats
 */
class EAN13Plugin : BarcodePlugin {
    
    override val name = "EAN-13"
    override val version = "1.0.0"
    override val supportedFormats = listOf(BarcodeFormat.EAN_13)
    override val priority = 100
    
    private val reader = MultiFormatReader().apply {
        setHints(mapOf(
            DecodeHintType.POSSIBLE_FORMATS to listOf(com.google.zxing.BarcodeFormat.EAN_13)
        ))
    }
    
    override fun canHandle(image: BufferedImage): Boolean {
        return try {
            // EAN-13 has specific dimensions and aspect ratio
            ...
            val result = reader.decode(bitmap)
              BarcodeResult(
                ...
                metadata = mapOf(
                    ...
                    "countryCode" to result.text.take(3),
                    "manufacturerCode" to result.text.substring(3, 8),
                    "productCode" to result.text.substring(8, 12),
                    "fullCode" to result.text
                )
            )
        } catch (e: NotFoundException) {
            throw BarcodeNotFoundException("No EAN-13 barcode found in image")
        } catch (e: Exception) {
            throw BarcodeDecodingException("Failed to decode EAN-13 barcode: ${e.message}")
        }
    }
      ...
      override fun getConfiguration(): Map<String, Any> {
        return mapOf(
            ...
            "features" to listOf(
                ...
                "Country/manufacturer identification"
            )
        )
    }
